...
module.exports.recordMatch = (payload) => {
    ...
    return Data.Find.pullFromMongo(findOptions, 'olympians')
    .then((resp) => {

    });

    return Data.Insert.insertToMongo(matchInfo, 'matches')
    .then((response) => {
        return Bluebird.map(matchInfo.players, (player) => {
            ...
            return Data.Find.pullFromMongo(findOptions, 'olympians')
            .then((response) => {
                newStats = _.clone(response.response[0]) || {};
                ...
                return Data.Update.updateRecord(findOptions, newStats, 'olympians');
            });
        });
    }).then(() => {
