...
internals.ssbScore = (kills, falls, games, points, damageTaken, damageGiven, randomCounter) => {
    ...
    multiplier = (kills / falls) * (damageGiven / damageTaken) * ((games / (games - randomCounter)) * 0.5);
...
internals.fullScore = (maggedGames, soberGames, fifaScore, ssbScore) => {
    ...
    return Math.ceil(((fifaScore + ssbScore) / 2 ) * (((maggedGames + soberGames) / soberGames) * 0.25) * 1000);
};
